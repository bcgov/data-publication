...
##### 1. Early Project Awareness
- You have AGO content (maps/apps/data...) destined for the public domain.
- Public release may require:
  - Metadata creation in the BC Data Catalogue (BCDC)
  - Formal approvals (Data Custodian, GCPE, Privacy)
  - Licensing and duplication review
- Early planning helps avoid delays during final publication.

##### 2. Permissions and Prerequisites
- At the beginning of the project, you need to be aware of the process to share the content to the public, which may include requirements for [_BC Data Catalogue_](https://catalogue.data.gov.bc.ca/dataset) (BCDC) metadata record(s).
- You need to have editing privileges in the BC Data Catalogue so you are able to create BCDC metadata record(s) for each of your AGO items that needs one.
- [How to become a provider or editor to the catalogue](./dps_bcdc_w_start.md#how-to-become-a-provider-to-the-catalogue)

##### 3. Create AGO Content in BC’s MapHub
- Create AGO content in [_B.C.’s Map Hub_](https://governmentofbc.maps.arcgis.com/home/index.html).

##### 4. Populate AGO Item Metadata
- Create Metadata in the AGO Item's details page. Refer to the [_METADATA GUIDELINES FOR AGO AUTHORS_](#metadata-guidelines-for-ago-authors) section below for more detailed information.
...
- Map
  ...
  - [BC's MapHub Public Content Publication Checklist – Map](./docs/BCs_MapHub_Public_Map_Checklist.docx)

- Application
  - If you are publishing a **web application** (e.g., Experience Builder apps), review and complete:
  - [BC's MapHub Public Content Publication Checklist – Application](./docs/BCs_MapHub_Public_Application_Checklist.docx)

##### 5. Data Custodian Authorization (Mandatory)
- Public release must be formally approved by the Data Custodian.
- The Data Custodian must confirm:
  - The data is appropriate for public release
  - Licensing permits public access
  - Any sensitivity or security concerns have been addressed

##### 6. Licensing and Duplication Review
- Before submitting a publication request, confirm the following.

##### 6.1 Licensing
- The dataset must be licensed as one of:
  - Open Government Licence – British Columbia, or
  - Access Only – Crown Copyright
- There must be no contractual or intellectual property restrictions preventing public access.

##### 6.2 Duplication
- Confirm the data does not duplicate authoritative data already available in:
  - [BC Geographic Warehouse (BCGW)](https://www2.gov.bc.ca/gov/content/data/finding-and-sharing/bc-geographic-warehouse)
...
+ When completing metadata, keep the following general principles in mind:
	- Titles, descriptions, and other metadata must be understandable to people outside government.
	- Titles must be descriptive enough that someone seeing them in a list (without viewing the full metadata) can assess the relevance of the item.
	- If data is hosted in BC’s MapHub, the metadata description must include a link to the corresponding BC Data Catalogue record that describes the dataset.  
  - This is done automatically for BCGW datasets that can be viewed through ArcGIS Online via a map service.
	+ **Title:** Ensure the title accurately describes the item.
		- Titles need to be descriptive enough so that someone seeing them in a list (without seeing the rest of the metadata) will be able to assess the relevance of the data to their needs.
	+ **Description section:** 
...
|AGO Hosted Data Layers (features, tiles...) - [**_Enhanced information_**](glossary.md#enhanced-information)|No (if AGO Item Details page sufficiently describes how the information was added/created and the sources from which it came so that an average _public_ user could access and recreate this enhanced information based on the information provided and _without too much complexity or effort_)|Yes - sufficiently described how the information was added/created and the sources from which it came so that an average _public_ user could access and recreate this enhanced information based on the information provided.  Referenced BCDC record(s) for source data.|
